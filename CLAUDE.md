...
48 SLURM scripts in `hpc/sweep/` generated by `hpc/sweep/generate_jobs.py`. All use `--route_dir` with 100 route variants per scenario to prevent OD-pair overfitting.

**Matrix:** 3 reward functions (queue, pressure, diff-waiting-time) x 3 learning rates (3e-3, 1e-3, 3e-4) x 2 scenarios = 18 base + 18 entropy annealing + 12 entropy coefficient sweep (pressure only, ent_coef 0.02/0.05/0.1) = **48 jobs**.

**Two-phase pipeline:**
1. `gen_train_routes.slurm` — generates 100 morning + 100 evening route variants
2. All 48 training scripts wait for routes, then run in parallel (128 CPUs, 300 episodes, 24h timeout)
