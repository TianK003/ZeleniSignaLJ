...
- **Route randomization** (`--route_dir` in experiment.py): Training on a single route file causes OD-pair overfitting — the model learns policies specific to the training traffic's origin-destination patterns and fails on unseen routes. `--route_dir` points to a directory of route file variants (generated by `generate_demand.py --scenario X --num_variants N`). With `--num_cpus > 1`, each subprocess independently picks its own random route file from the directory (keyed by PID) at env creation time, giving diverse traffic patterns within each rollout. In pure route-rotation mode (no curriculum), envs are NOT forcibly reset between PPO updates — episodes play out fully so the agent sees the complete rush-hour period. In curriculum mode, `model._last_obs = None` forces an env reset to load new demand files. Compatible with `--curriculum` for combined hour + route randomization.
...
| `n_steps` | 720 | Steps per environment before PPO update |
| `batch_size` | 180 (base) | Auto-scaled to `180 * num_cpus` at runtime to maintain ~20 mini-batches/epoch |
...
```
1 episode (uniform, 1h) = rl_seconds / delta_time * num_agents
                        = 3600 / 5 * 5 = 3600 SB3 timesteps

1 episode (morning_rush, 4h) = 14400 / 5 * 5 = 14400 SB3 timesteps
```

Steps_per_episode is computed dynamically from `args.num_seconds` in experiment.py, not hardcoded from `RL_SECONDS`.

### HPC Parallelization (--num_cpus)

With `--num_cpus N`, experiment.py creates N independent SUMO simulations via `SubprocVecEnv`. Each has 5 agents, so PPO sees `N * 5` parallel agents. Key scaling behaviors:

- **Rollout buffer**: `n_steps * N * 5` transitions per PPO update (e.g., 720 * 128 * 5 = 460,800 with 128 CPUs)
- **Batch size auto-scaling**: `batch_size = 180 * num_cpus` to maintain ~20 mini-batches per epoch. Without this, 128 CPUs would cause 2,560 mini-batches/epoch (128x too many gradient updates), leading to overfitting each rollout buffer.
- **Episode counting**: `ppo_updates = ceil(episode_count / num_cpus)` since each PPO update collects from num_cpus parallel SUMO episodes. With `--episode_count 300 --num_cpus 128`: 3 PPO updates (not 300).
- **Route diversity**: With `--route_dir`, each subprocess independently picks its own random route file (keyed by PID), giving all N workers different traffic patterns within the same rollout.
- **No forced reset in pure route-rotation mode**: SUMO episodes play out fully across multiple rollouts so the agent sees the complete rush-hour period (not just the first 25%). Only curriculum mode forces env reset (to load new demand files).

| `--episode_count` | `--num_cpus` | PPO updates | Buffer size | Actual batch_size |
|-------------------|--------------|-------------|-------------|-------------------|
| 50 | 1 | 50 | 3,600 | 180 |
| 50 | 4 | 13 | 14,400 | 720 |
| 300 | 128 | 3 | 460,800 | 23,040 |
| 500 | 128 | 4 | 460,800 | 23,040 |
...
### Sweep 2 (completed): Route-randomized training

44 SLURM scripts in `hpc/sweep/` generated by `hpc/sweep/generate_jobs.py`. All use `--route_dir` with 50 route variants per scenario to prevent OD-pair overfitting.
...
**HPC parallelization (128 CPUs):**
- `batch_size` auto-scaled to `180 * 128 = 23,040` (maintains ~20 mini-batches/epoch, same as single-CPU)
- `--episode_count 300` → `ceil(300/128) = 3` PPO updates (each collecting 128 parallel episodes)
- Each subprocess independently selects its own route file from `--route_dir` for within-batch diversity
- SUMO episodes play out fully (no forced reset between PPO updates in pure route-rotation mode)
- `PYTHONUNBUFFERED=1` set in `hpc/common.sh` for immediate SLURM log output

**Checkpoints:** Saved at regular intervals as `checkpoints/ppo_policy_Nep.zip` with companion `.json` metadata (ppo_update, equivalent_episodes, timesteps, avg reward, baseline comparison). Emergency `ppo_model_latest.zip` always saved via try/finally even on crash/timeout. Final model: `ppo_shared_policy.zip`.
...
- `--route_dir DIR` — directory of route file variants. Each subprocess picks its own random route (per-worker diversity). In curriculum mode, route also changes per PPO update.
- `--ent_coef FLOAT` — overrides config.py ENT_COEF
- `--entropy_annealing` — linearly anneal entropy from ent_coef to 0.01 over training
- `--episodes_per_save N` — checkpoint frequency (default: 10, scaled to PPO-update units)
...
## Top Policy Selection (Sweep 2 — Route-Randomized)

After the second HPC sweep (44 configurations x 3200 episodes on 16 CPUs, route-randomized training with 50 route variants per scenario), the best models were selected by single-run improvement % over fixed-time baseline on the training scenario.

**Best Morning Rush model:**
- Tag: `morningrush_diffwaitingtime_lr1e3_3200ep`
- Model path: `results/experiments/20260413_005828_66186002_morningrush_diffwaitingtime_lr1e3_3200ep/ppo_shared_policy.zip`
- Reward function: diff-waiting-time, LR: 1e-3
- Single-run training improvement: +1.57%
- Maps to: M2 in rush generalization tests

**Best Evening Rush model:**
- Tag: `eveningrush_diffwaitingtime_lr1e3_3200ep`
- Model path: `results/experiments/20260413_005825_66185980_eveningrush_diffwaitingtime_lr1e3_3200ep/ppo_shared_policy.zip`
- Reward function: diff-waiting-time, LR: 1e-3
- Single-run training improvement: +8.16%
- Maps to: E1 in rush generalization tests

### Rush-Hour Generalization Test Results (50-seed paired t-test)

Isolated 4h rush-hour tests with 50 different route files per model. Tests whether trained models generalize across random OD pairs. Results from `results/statistical-test/`.

**M2_morning (best morning):**
- 50-seed overall reward: -1.99% vs baseline (slightly worse)
- Wait time improvement: +1.84%
- Paired t-test: p=0.092 (not significant at alpha=0.05)
- Cohen's d = -0.243 (small effect)
- Per-intersection: Askerceva +18.48%, Pivovarna -0.12%, Slovenska -5.08%, Trzaska -14.14%, Kolodvor -22.13%

**E1_evening (best evening):**
- 50-seed overall reward: -1.11% vs baseline (slightly worse)
- Wait time improvement: +4.29%
- Paired t-test: p=0.187 (not significant at alpha=0.05)
- Cohen's d = -0.189 (small effect)
- Per-intersection: Askerceva +13.40%, Pivovarna +1.80%, Kolodvor -2.65%, Slovenska -13.35%, Trzaska -14.13%

**Key finding:** Route-randomized models (Sweep 2) show much better generalization than single-route models (Sweep 1, which degraded -15% to -27% on unseen routes). Sweep 2 models are roughly baseline-equivalent on reward with modest wait time improvements, but the improvement is not statistically significant. The models consistently improve Askerceva while degrading Trzaska, suggesting the shared-policy architecture struggles to optimize all 5 intersections simultaneously.
